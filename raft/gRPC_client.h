...
// TODO: introduce stub and channel caching to reduce overhead when
// communicating to nodes
namespace raft {
enum class RPCType {
  REQUEST_VOTE,
  APPEND_ENTRIES,
...
public:
  ...
  void
  append_entries(std::string address, const AppendEntriesRPC &append_entries,
                 std::function<void(AppendEntriesResponse)> callback) override;

private:
  void handle_request_vote_rpc(
      const AsyncCall<RequestVoteResponse, raft_gRPC::RequestVoteResponse>
          *call,
      bool ok) const;

  void handle_append_entries_rpc(
      const AsyncCall<AppendEntriesResponse, raft_gRPC::AppendEntriesResponse>
          *call,
      bool ok) const;
